...
describe('aggregate', function() {
  ...
  describe('{$group: "Gender"}', function() {
    it('should return grouped result based on Gender', function() {
      const p3Opts = Object.keys(p3OptsToLodashOpts);
      const optsTotal = p3Opts.length;

      for(var i = 0; i < optsTotal; i++ ) {
        assert.deepEqual(
          runP3('Gender', p3Opts[i]),
          runLodash('Gender', p3OptsToLodashOpts[p3Opts[i]])
        );
      }
    });
  });
  ...
  describe('{$group: "MotherRace"}', function() {
    it('should return grouped result based on MotherRace', function() {
      ...
      for(var i = 0; i < optsTotal; i++ ) {
        assert.deepEqual(
          runP3('MotherRace', p3Opts[i]),
          runLodash('MotherRace', p3OptsToLodashOpts[p3Opts[i]])
        );
      }
    });
  });
